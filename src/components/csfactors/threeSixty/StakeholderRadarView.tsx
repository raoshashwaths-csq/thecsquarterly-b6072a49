...
import { AnalyticsEmpty } from "@/components/analytics/AnalyticsShell";
...
function avg(accounts: CSAccount[]): Record<AxisKey, number> {
  ...
  const sums = accounts.map(scoreAccount).reduce(
    (acc, s) => { for (const k of Object.keys(s) as AxisKey[]) acc[k] += s[k]; return acc; },
...
export function StakeholderRadarView() {
  const { user } = useAuth();
  ...
  const compositeFocus = Math.round((Object.values(focus).reduce((s, v) => s + v, 0)) / AXES.length);

  const stakeholders = useMemo(
    () => [...accounts].sort((a, b) => a.health - b.health).slice(0, 6),
    [accounts],
  );

  if (isLoading) return <p className="text-sm text-muted-foreground py-12">Loading…</p>;
  if (accounts.length === 0) return <AnalyticsEmpty message="No accounts yet. Add accounts in CSFactors to plot stakeholder radar." />;
  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-[1fr,1fr] gap-6 mt-8">
        <SectionCard eyebrow="Radar" title={selected ? selected.name : "Portfolio average"} description="Solid = current focus. Dashed = portfolio benchmark.">
          <div className="flex flex-col items-center">
            <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-full max-w-[360px] h-auto">
              ...
              {AXES.map((axis, i) => {
                const a = (Math.PI * 2 * i) / AXES.length - Math.PI / 2;
                return (
                  <line key={axis.key} x1={cx} y1={cy} x2={cx + Math.cos(a) * R} y2={cy + Math.sin(a) * R} className="stroke-border" strokeWidth={0.5} />
                );
              })}
              {selected && (
                <polygon points={polygonPoints(portfolio, cx, cy, R)} className="fill-secondary-accent/10 stroke-secondary-accent" strokeWidth={1.5} strokeDasharray="4 3" />
              )}
              <polygon points={polygonPoints(focus, cx, cy, R)} className="fill-accent/25 stroke-accent" strokeWidth={2} />
              {AXES.map((axis, i) => {
                const a = (Math.PI * 2 * i) / AXES.length - Math.PI / 2;
                const x = cx + Math.cos(a) * (R + 22);
                const y = cy + Math.sin(a) * (R + 22);
                return (
                  <text key={axis.key} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="fill-foreground font-mono uppercase tracking-widest text-xs">
                    {axis.label}
                  </text>
                );
              })}
            ...
            {selected && (
              <button type="button" onClick={() => setSelectedId(null)} className="mt-4 font-mono uppercase tracking-widest text-xs text-muted-foreground hover:text-accent">
        ...
        <SectionCard eyebrow="Accounts" title="Pick a stakeholder" description="Sorted by lowest health first — your highest-leverage attention.">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-border border border-border">
          {[
            ...
            <div key={i} className="bg-card p-5">
              <div className="font-mono text-xs uppercase tracking-[0.25em] text-secondary-accent mb-2">{m.tag}</div>
