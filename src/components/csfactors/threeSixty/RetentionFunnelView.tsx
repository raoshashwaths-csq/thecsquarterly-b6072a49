...
const STAGES = [
  { key: "Signed", label: "Signed", match: (_a: CSAccount) => true },
...
export function RetentionFunnelView() {
  const { user } = useAuth();
  ...
  if (accounts.length === 0) return <AnalyticsEmpty message="No accounts yet. Add accounts in CSFactors to see retention stages." />;
  ...
  return (
    <>
      ...
      <SectionCard eyebrow="Funnel" title="Stage-by-stage progression" description="Each bar is sized to the count of accounts at that stage. Width = retention vs. previous stage." className="mt-8">
              ...
              <div key={s.key} className="space-y-1.5">
                <div className="flex items-baseline justify-between gap-3 text-sm">
                  <div className="flex items-baseline gap-3 min-w-0">
                    <span className="font-mono text-xs uppercase tracking-[0.25em] text-muted-foreground w-6 shrink-0">0{i + 1}</span>
                ...
                <div className="h-9 bg-muted/40 relative overflow-hidden">
                  <div className="h-full bg-accent transition-all duration-700 ease-out flex items-center justify-end pr-3" style={{ width: `${widthPct}%` }}>
                    {widthPct > 15 && (
                      <span className="font-mono uppercase tracking-widest text-xs text-accent-foreground">{stageRetention}%</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <SectionCard eyebrow="Diagnosis" title="Where they leak" description="The largest stage-over-stage drop is your highest-leverage fix." className="mt-6">
          ...
          {stages.slice(1).map((s, i) => {
            ...
            return (
              <div key={s.key} className="bg-card p-5">
                <div className="font-mono text-xs uppercase tracking-[0.25em] text-muted-foreground mb-2">{prev.label} → {s.label}</div>
                <div className="font-display text-3xl tracking-tight mb-1 tabular-nums">{dropPct}%</div>
