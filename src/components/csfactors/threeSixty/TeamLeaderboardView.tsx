...
type Row = { name: string; count: number; arr: number; avgHealth: number; qbrPct: number; atRiskArr: number };
...
function aggregate(accounts: CSAccount[]): Row[] {
  ...
  for (const [name, list] of map) {
    ...
    rows.push({ name, count: list.length, arr, avgHealth, qbrPct: Math.round((qbr / list.length) * 100), atRiskArr: atRisk });
  }
...
export function TeamLeaderboardView() {
  ...
  const movers = useMemo(() => [...rows].sort((a, b) => b.avgHealth - a.avgHealth).slice(0, 3), [rows]);
  ...
  if (rows.length === 0) return <AnalyticsEmpty message="No CSM data yet. Assign CSMs to accounts in CSFactors to populate the leaderboard." />;
  ...
  return (
    <>
      ...
      <SectionCard eyebrow="Leaderboard" title="Ranked by book of business" description="Tap a column header to sort. Health and QBR are aggregated across the CSM's accounts." className="mt-8">
        <div className="overflow-x-auto -mx-6 px-6">
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="text-left border-b border-border">
                {["#", "CSM", "Accts", "Book", "Avg Health", "QBR %", "ARR at Risk"].map((h, i) => (
                  <th key={h} className={`py-3 font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground ${i === 0 ? "w-10" : ""}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.name} className="border-b border-border/60 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="py-3"><div className="font-display text-base">{r.name}</div></td>
