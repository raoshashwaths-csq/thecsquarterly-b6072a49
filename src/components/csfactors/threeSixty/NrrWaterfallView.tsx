...
export function NrrWaterfallView() {
  const { user } = useAuth();
  ...
  if (accounts.length === 0) return <AnalyticsEmpty message="No accounts yet. Add accounts in CSFactors to model NRR." />;
  ...
  return (
    <>
      ...
      <SectionCard eyebrow="Waterfall" title="Movement of ARR across the period" description="Green = expansion. Gold = contraction. Oxblood = churn." className="mt-8">
          ...
          {cols.map((c) => {
            ...
            return (
              <div key={c.label} className="flex flex-col items-center h-full justify-end">
                <div className="font-mono text-xs md:uppercase tracking-widest tabular-nums mb-1 text-center">
                ...
                <div className="font-mono text-xs md:uppercase tracking-widest text-muted-foreground mt-2 text-center leading-tight">
                  {c.label}
                </div>
              </div>
            );
      ...
      <SectionCard eyebrow="Cadence" title="NRR rhythm — last 8 periods" description="Trailing NRR (modeled) leading into this snapshot." className="mt-6">
        <RhythmBars values={rhythm} labels={["Q1", "Q2", "Q3", "Q4", "Q1", "Q2", "Q3", "Now"]} />
      </SectionCard>
    </>
  );
}
